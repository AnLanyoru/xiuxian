...
class MixtureData:
    ...
    def send_back(self, item_id: int, need_items_id: list, need_items_num: list,
        ...
        if table:
            # 判断是否存在，存在则update
            sql = f"""INSERT INTO back (item_id, need_items_id, need_items_num, update_time, state, is_bind_mixture)
                VALUES (?,?,?,?,?,?)"""
            cur.execute(sql, (item_id, need_items_id, need_items_num, now_time, state, is_bind_mixture))
        else:
            # 判断是否存在，不存在则INSERT
            sql = f"""INSERT INTO back (item_id, need_items_id, need_items_num, create_time, update_time, state, is_bind_mixture)
                VALUES (?,?,?,?,?,?,?)"""
