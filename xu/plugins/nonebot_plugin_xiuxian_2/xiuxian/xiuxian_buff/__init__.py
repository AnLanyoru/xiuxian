...
stone_exp = on_command("灵石修仙", aliases={"灵石修炼", "/灵石修炼"}, priority=5, permission=GROUP, block=True)
two_exp = on_command("双修", priority=5, permission=GROUP, block=True)
mind_state = on_command("我的状态", aliases={"/我的状态"}, priority=7, permission=GROUP, block=True)
select_state = on_command("查看状态", aliases={"查状态"}, priority=7, permission=GROUP, block=True)
qc = on_command("切磋", priority=6, permission=GROUP, block=True)
blessed_spot_create = on_command("洞天福地购买", aliases={"获取洞天福地", "购买洞天福地"}, priority=10, permission=GROUP, block=True)
blessed_spot_info = on_command("洞天福地查看", aliases={"我的洞天福地", "查看洞天福地"}, priority=11, permission=GROUP, block=True)
blessed_spot_rename = on_command("洞天福地改名", aliases={"改名洞天福地", "改洞天福地名"}, priority=7, permission=GROUP, block=True)
...
突破状态: {exp_meg}
(概率：{jsondata.level_rate_data()[user_msg['level']] + leveluprate + number}%)
攻击修炼:{user_msg['atkpractice']}级
(提升攻击力{user_msg['atkpractice'] * 4}%)
